    ...
    for r in driver.requests:
        if r.response and ".m3u8" in r.url:
            ...
            if any(k in r.url for k in ["2000000", "2500000", "3000000", "4000000", "hd", "high"]):
                ...
                print(f"[台灣頻道/{channel_name}] 🎥 檢測到高清流: {r.url}")
            else:
                print(f"[台灣頻道/{channel_name}] ⚠️ 檢測到低碼率流: {r.url}")

    driver.quit()

    if candidates:
        stream_url = candidates[-1]  # 取最後一個，通常是最高碼率
        group_dir = os.path.join(OUTPUT_DIR, "台灣頻道")
        os.makedirs(group_dir, exist_ok=True)
        output_file = os.path.join(group_dir, f"{channel_name}.m3u")
        with open(output_file, "w", encoding="utf-8") as f:
            f.write(f"#EXTM3U\n#EXTINF:-1 group-title=\"台灣頻道\",{channel_name}\n{stream_url}\n")
        print(f"[台灣頻道/{channel_name}] ✅ 已保存高清直播源")
        return stream_url
    else:
        print(f"[台灣頻道/{channel_name}] ⚠️ 未检测到高清直播流")
        return None
...
def update_all_channels():
    print(f"\n🕒 [{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 開始更新台灣頻道...")
    for name, url in CHANNELS_TAIWAN.items():
